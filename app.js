...
function listofresult(event) {
    ...
    for (let i = 0; i < CAtalogbusmall.busmall.length; i++) {
        ...
        listImage.textContent = `${CAtalogbusmall.busmall[i].name}  had ${CAtalogbusmall.busmall[i].counterclic} , and was seen ${CAtalogbusmall.busmall[i].times} times.`;
...
function gitProduct() {
    let stringdata = localStorage.getItem('CAtalogbusmall');
    let dataofopj = JSON.parse(stringdata);
    if (dataofopj !== null) {
        CAtalogbusmall.busmall = dataofopj;
    }
